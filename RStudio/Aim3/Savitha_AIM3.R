...
metadata <- read_delim("hiv_metadata.tsv")
...
# Generate pathway PCA plot ### DOESNT WORK
pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = metadata, group = "Visit_Age")
...
png("age_barplot.png", width = 2200, height = 1800, res = 150)
...
# Run the function on your own data
labour_res =  DEseq2_function(labour_abundance_daa_results_df, metadata, "Labor_category")
...
png("labour_barplot.png", width = 2200, height = 1800, res = 150)
ggplot(data = labour_sig_res, aes(y = reorder(description, sort(as.numeric(log2FoldChange))), x= log2FoldChange, fill = pvalue))+
  geom_bar(stat = "identity")+ 
  theme_bw()+
  labs(x = "Log2FoldChange", y="Pathways")
dev.off()
