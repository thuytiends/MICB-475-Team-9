...
hiv_fin <- subset_samples(hiv_final, Visit_Cat == "2nd Visit")
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), 
                                        metadata = metadata_filtered, group = "Visit_Age", daa_method = "DESeq2")
...
metadata$Visit_Age <- ifelse(metadata$Visit_Age > 45, "Older than 45", "Younger than 45")
metadata = metadata[!is.na(metadata$Visit_Age),]
summary(metadata$Visit_Age) 
table(metadata$Visit_Age > 45, useNA = "always") 
sum(is.na(metadata$Visit_Age))
unique(metadata$Visit_Age)
...
res =  DEseq2_function(abundance_data_filtered, metadata_filtered, "Visit_Age")
...
sig_res = res_desc %>%
  filter(pvalue < 0.05, abs(log2FoldChange) >= 1.5)  # Added log2FC filter
